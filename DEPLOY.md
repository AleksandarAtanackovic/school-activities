# Праћење ваннаставних секција — Упутство за постављање (cPanel)

Самостална PHP + MySQL веб апликација за праћење ваннаставних секција у
средњој школи. Без фрејмворка, без билд корака, без спољних сервиса — ради на
сваком стандардном cPanel/Apache + MySQL хостингу.

Ово је **Фаза 1 (Milestone 1)**: налози и улоге, секције, додела наставника,
...
3. Изаберите SQL фајл → **Go**:
   ...
   - **`schema.sql`** — демо подаци (примери корисника, секције, пријава) за
...
**Администратор** — види и мења све: креира/уређује секције, додељује једног
или више наставника свакој секције, поставља капацитет и распоред, одобрава/
одбија све пријаве, управља корисницима.

**Наставник** — види само секције којима је додељен; уређује њихове детаље,
...
**Ученик** — прегледа отворене секције, пријављује се (једна пријава по
секције), се пријављује корисничким именом, има матични број, прати статус пријаве, има страницу „Моје ваннаставне секције“ и
обједињени списак обавештења из секције у које је уписан.
